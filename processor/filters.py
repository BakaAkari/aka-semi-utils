...
class WatermarkFilter(FilterProcessor):
    ...
    def process(self, ctx: PipelineContext):
        ...
        # --- Framed layout (白边相框) ---
        if ctx.get("layout_mode") == "framed":
            self._process_framed(ctx, img)
            return
    ...
    # --- Framed layout rendering (白边相框) ---

    def _process_framed(self, ctx: PipelineContext, img: Image.Image) -> None:
        """Render the white-border frame + structured info bar layout."""
        from core.image_io import load_logo
        from PIL import ImageDraw, ImageFont

        lm = ctx.getint("left_margin", 40)
        rm = ctx.getint("right_margin", 40)
        tm = ctx.getint("top_margin", 40)
        bm = ctx.getint("bottom_margin", 120)  # = border + bar height
        bg_color = ctx.get("color", "#FFFFFF")

        canvas_w = img.width + lm + rm
        canvas_h = img.height + tm + bm
        canvas = Image.new("RGBA", (canvas_w, canvas_h), bg_color)

        # Paste the photo
        canvas.paste(img, (lm, tm), mask=img if img.mode == "RGBA" else None)

        # Info bar area: from photo bottom to canvas bottom
        bar_y = tm + img.height
        bar_h = bm - ctx.getint("frame_border_width", 40)  # info bar height
        border_w = lm  # border width (all sides equal)

        # --- Load logo ---
        logo_img = None
        logo_path = ctx.get("frame_logo", "")
        if logo_path:
            try:
        ...
        # --- Render texts ---
        primary_text = ctx.get("frame_left_primary", "")
        secondary_text = ctx.get("frame_left_secondary", "")
        params_text = ctx.get("frame_params", "")

        color_primary = ctx.get("frame_text_primary", "#333333")
        color_secondary = ctx.get("frame_text_secondary", "#888888")

        # Font: try system fonts, fall back to project fonts
        font_paths = [
        ...
        try:
            ...
            # Use the project fonts from shared path
            import os as _os
            project_font = _os.path.join(_os.path.dirname(__file__), "..", "config", "fonts", "NotoSansCJKsc-Bold.otf")
            if _os.path.exists(project_font):
        ...
        primary_h = max(10, int(bar_h * 0.30))
        secondary_h = max(8, int(bar_h * 0.22))
        params_h = max(8, int(bar_h * 0.22))

        # Override with user ratio if set
        primary_ratio_raw = ctx.get("frame_primary_ratio", 0)
        try:
            primary_ratio = float(primary_ratio_raw or 0)
        except (TypeError, ValueError):
            primary_ratio = 0.0
        if primary_ratio > 0:
            primary_h = int(bar_h * primary_ratio / 0.30 * 0.30)
        params_ratio_raw = ctx.get("frame_params_ratio", 0)
        try:
            params_ratio = float(params_ratio_raw or 0)
        except (TypeError, ValueError):
            params_ratio = 0.0
        if params_ratio > 0:
            params_h = int(bar_h * params_ratio / 0.22 * 0.22)
        ...
        # Spacing
        gap = max(4, int(bar_h * 0.08))
        logo_h = max(10, int(bar_h * 0.45))

        # --- Right side: logo ---
        right_x = canvas_w - border_w - 4
        if logo_img and logo_img.height > 0:
            logo_w = max(1, int(logo_img.width * logo_h / logo_img.height))
            logo_resized = logo_img.resize((logo_w, logo_h), Image.Resampling.LANCZOS)
            logo_x = right_x - logo_w
            logo_y = bar_y + border_w + (bar_h - border_w * 2 - logo_h) // 2
            canvas.paste(
                logo_resized,
                (logo_x, logo_y),
                mask=logo_resized if logo_resized.mode == "RGBA" else None,
            )
            right_x = logo_x - gap * 3

        # --- Left side: primary (model) + secondary (lens), stacked ---
        left_x = border_w + gap
        text_y = bar_y + border_w + gap

        if primary_text:
            font = _load_font(primary_h)
            draw.text((left_x, text_y), primary_text, fill=color_primary, font=font)
            bbox = draw.textbbox((0, 0), primary_text, font=font)
            text_y += (bbox[3] - bbox[1]) + gap // 2

        if secondary_text:
            font = _load_font(secondary_h)
            draw.text((left_x, text_y), secondary_text, fill=color_secondary, font=font)

        # --- Bottom: params line, right-aligned ---
        if params_text:
            font = _load_font(params_h)
            bbox = draw.textbbox((0, 0), params_text, font=font)
            text_w = bbox[2] - bbox[0]
            params_x = canvas_w - border_w - gap - text_w
            params_y = bar_y + bar_h - border_w - gap - (bbox[3] - bbox[1])
            draw.text((params_x, params_y), params_text, fill=color_secondary, font=font)

        # Signature line
        sig_text = ctx.get("custom_text", "")
        if sig_text:
            font = _load_font(int(params_h * 0.85))
            sig_label = f"\u00a9 {sig_text}"
            bbox = draw.textbbox((0, 0), sig_label, font=font)
            sig_w = bbox[2] - bbox[0]
            sig_x = canvas_w - border_w - gap - sig_w
            sig_y = bar_y + bar_h - border_w - gap
            draw.text((sig_x, sig_y), sig_label, fill=color_secondary, font=font)
